...
public class CarePlanService {
    ...
    public Optional<CarePlan> getSuggestion(String plantName) {
        List<CarePlan> results = carePlanRepository.findByPlantTypeContainingIgnoreCase(plantName);
        // Prefer exact match first
        return results.stream()
            .filter(c -> c.getPlantType().equalsIgnoreCase(plantName))
            .findFirst()
            .or(() -> results.stream().findFirst());
    }

    public List<CarePlan> searchCarePlans(String plantName) {
        String cleaned = plantName.replaceAll("[^a-zA-Z]", "").toLowerCase();
        List<CarePlan> all = carePlanRepository.findAll();
        return all.stream()
            .filter(c -> {
                String type = c.getPlantType().replaceAll(" ", "").toLowerCase();
                return type.contains(cleaned) || cleaned.contains(type) || similarity(type, cleaned) > 0.4;
            })
